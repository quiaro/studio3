...
public class GridFSServiceImpl implements GridFSService {
    ...
    @Override
    public GridFSFile saveFile(final String fileName, final InputStream file) throws MongoRepositoryException {
        try {
            return gridFsTemplate.store(file, fileName);
        } catch (MongoException ex) {
            log.error("Unable to save File {} due a error {} ", fileName, ex);
